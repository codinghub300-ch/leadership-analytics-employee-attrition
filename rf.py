...
# ============================================
# Classify Leaders as Good / Not Good
# ============================================
threshold = leader_df['LeaderScore'].quantile(0.6)  # top 40% Good
leader_df['LeaderQuality'] = np.where(leader_df['LeaderScore'] >= threshold, 'Good', 'Not Good')

good_count = (leader_df['LeaderQuality'] == 'Good').sum()
not_good_count = (leader_df['LeaderQuality'] == 'Not Good').sum()

print(f"\nTotal Good Leaders: {good_count}")
print(f"Total Not Good Leaders: {not_good_count}")
...
plt.bar(leader_df['ManagerID'], leader_df['LeaderScore'], 
        color=['green' if x=='Good' else 'red' for x in leader_df['LeaderQuality']])
plt.xticks(rotation=45)
plt.title("Leader Ranking (AI-Based)")
plt.tight_layout()
plt.show()
...
# Save
# ============================================
leader_df.to_csv("leader_analysis_real_ai.csv", index=False)
